...
app.get('/api/coupons', async (req, res) => {
  ...
  const coupons = await db.all("SELECT *, (usage_limit > 0 AND times_used >= usage_limit) as exhausted FROM coupons WHERE active=1 AND (expires_at IS NULL OR expires_at > ?) AND code NOT LIKE 'FIDEL-%' ORDER BY id ASC", now);
  res.json(coupons);
});
  ...
  try {
    ...
    if (phone) {
      ...
      if (currentCount >= loyaltyGoal) {
        // Tenta resetar atomicamente — só quem conseguir (changes > 0) gera o prêmio
        const result = await db.run('UPDATE loyalty SET count = 0 WHERE phone=? AND count >= ?', phone, loyaltyGoal);
        if (result.changes > 0) {
